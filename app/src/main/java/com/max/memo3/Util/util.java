...
//            init_Sensor();
    ...
    public static void makeToast(CharSequence theThing){makeToast(APP_CONTEXT,theThing);}
    ...
    public static void makeToastLog(SERIOUS_LEVEL level, String message){makeToastLog(APP_CONTEXT,level,message);}
...
//            log("snackbar dismiss");
    ...
    public static class NotiBuilder{
        ...
        public NotiBuilder setOnPressAction(String action, Class<? extends BroadcastReceiver> BRclass){return setOnPressAction(action, BRclass,null);}
        public NotiBuilder setOnPressAction(String action){return setOnPressAction(action, MEMO3_BroadcastReceiver.class, null);}
        public NotiBuilder setOnPressAction(String action,Bundle extraBundle){return setOnPressAction(action,MEMO3_BroadcastReceiver.class,extraBundle);}
    ...
    public static void makeVibrate_pattern(long[] ms, int[] amplitude, int repeat){
        if (ms.length!=amplitude.length){return;}
        stopVibrator();
        vibrator.vibrate(VibrationEffect.createWaveform(ms,amplitude,repeat));
    }
    public static void makeVibrate_pattern(long[] ms, int[] amplitude){makeVibrate_pattern(ms,amplitude,-1);}
    public static void makeVibrate_pattern(long[] ms, int repeat){
        stopVibrator();
        vibrator.vibrate(VibrationEffect.createWaveform(ms,repeat));
    }
    public static void makeVibrate_pattern(long[] ms){makeVibrate_pattern(ms,-1);}
    ...
    public static void makeSchedule_short(long delayms, Scheduler_onFinish onFinish, long intervalms, Scheduler_onTick onTick){
        new CountDownTimer(delayms,intervalms){
            @Override public void onTick(long millisUntilFinished) {onTick.onTick(millisUntilFinished);}
            @Override public void onFinish() {onFinish.onFinish();}
        }.start();
    }
    ...
    public static Timer makeSchedule_long(long delayms, Scheduler_onFinish onFinish, long periodms){
        ...
        timer.schedule(new TimerTask() {@Override public void run() {onFinish.onFinish();}},delayms,periodms);
        return timer;
    }
    public static void makeSchedule_long(long delayms, Scheduler_onFinish onFinish){
        new Timer(true).schedule(new TimerTask() {@Override public void run() {onFinish.onFinish();}},delayms);
    }
    ...
    public static Timer makeSchedule_long(Calendar triggerTime, Scheduler_onFinish onFinish, long periodms){
        if (!isTimeAfterNow(triggerTime)){return null;}
    ...
    public static void makeSchedule_long(Calendar triggerTime, Scheduler_onFinish onFinish){
        if (!isTimeAfterNow(triggerTime)){return;}
    ...
    public static void makeSchedule_long(int year_start, int month_start, int day_of_month_start, int hour_start, int minute_start, int second_start,
        ...
        if (!isTimeAfterNow(startCalendar)){return;}
        if (!stopCalendar.after(startCalendar)){return;}
        new Thread(() -> {
    ...
    public interface Scheduler_onTick{void onTick(long millisUntilFinished);}
    public interface Scheduler_onFinish{void onFinish();}
    ...
    public static void makeWork_cancel(UUID uuid){workManager.cancelWorkById(uuid);}
    public static void makeWork_cancel(String uuid_string){makeWork_cancel(UUID.fromString(uuid_string));}
    public static void makeWork_cancelAll(){workManager.cancelAllWork();}

    //CALENDAR + TIME
    public static long getCurrTime_ms(){return System.currentTimeMillis();}
    public static Calendar getCurrTime_calendar(){return new Calendar.Builder().setInstant(new Date(getCurrTime_ms())).build();}
    public static Date getCurrTime_date(){return getCurrTime_calendar().getTime();}
    public static String getCurrTime_string(){return getCurrTime_date().toString();}
    ...
    public static boolean isTimeAfterNow(Calendar time){return time.after(getCurrTime_calendar());}
    public static boolean isTimeBeforeNow(Calendar time){return time.before(getCurrTime_calendar());}
    ...
    public static void makeFlash_ON(){makeFlash(true);}
    public static void makeFlash_OFF(){makeFlash(false);}
    public static void makeFlash_toggle(){
        flashOnNow = !flashOnNow;
        makeFlash(flashOnNow);
    }
    public static boolean isFlashOn(){return flashOnNow;}
    ...
    public static int makeRuntimePacket_send(Object thingToSend){
        ...
        if (packetInTransport_runtime.containsKey(index)){return makeRuntimePacket_send(thingToSend);}
...
//            ConnectivityManager.CONNECTIVITY_ACTION
    ...
    public static void makeLocalBR_register(String action, LocalBRrun whatToRun){
        if (registeredLocalBRActions.containsKey(action)){return;}
    ...
    public interface LocalBRrun {void run(Context context, Intent intent);}

    //BROADCAST RECEIVER (all time)
    //TODO need to do this?
    public static Class getBR_class(){return MEMO3_BroadcastReceiver.class;}
...
//                .deleteRealmIfMigrationNeeded()     //for developing use
    ...
    private static final int googleSignInActivityRequestCode = 69;
    public static GoogleSignInAccount getGoogleAccount(){return googleAccount;}
    ...
    public static void makeGoogleSignIn_signOut(){googleSignInClient.signOut();}
    public static void makeGoogleSignIn_disconnect(){googleSignInClient.revokeAccess();}
    public static boolean isGoogleSignIned(){return googleAccount!=null;}
    ...
    public static void makeFirebase_signOut(){FirebaseAuth.getInstance().signOut();}
    public static boolean isFirebaseSignIned(){return firebaseUser!=null;}
    ...
    //SERVICE
    //comment because currently no use
